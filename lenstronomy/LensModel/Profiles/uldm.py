...
class Uldm(LensProfileBase):
    ...
    :param inverse_theta_c: INVERSE core radius (in arcseconds)
    :param slope: exponent entering the profile, default value is 8
    """
    _s = 0.000001  # numerical limit for minimal radius
    param_names = ['kappa_0', 'inverse_theta_c', 'slope', 'center_x', 'center_y']
    lower_limit_default = {'kappa_0': 0, 'inverse_theta_c': 0, 'slope': 3.2, 'center_x': -100, 'center_y': -100}
    upper_limit_default = {'kappa_0': 1., 'inverse_theta_c': 100, 'slope': 9, 'center_x': 100, 'center_y': 100}

    def rhotilde(self, kappa_0, inverse_theta_c, slope = 8):
        """
        Computes the central density in angular units
        :param kappa_0: central convergence of profile
        :param inverse_theta_c: core radius (in arcsec)
        :param slope: exponent entering the profile
        :return: central density in 1/arcsec
        """
        #  a_factor_sqrt = np.sqrt( (0.5)**(-1/slope) -1)
        num_factor = gamma(slope) / gamma(slope - 1/2)  / np.sqrt(np.pi)
        return kappa_0 * num_factor * inverse_theta_c

    def function(self, x, y, kappa_0, inverse_theta_c, slope=8, center_x=0, center_y=0):
        """
        ...
        if np.isscalar(r) == True:
            hypgeom = float(kappa_0 /2 * r**2 * hyp3f2(1, 1, slope - 0.5, 2, 2, -(r *inverse_theta_c )**2))
        else:
            hypgeom =  np.array([ kappa_0 /2. * r_i**2. *
                hyp3f2(1, 1, slope - 0.5, 2, 2, -(r_i * inverse_theta_c)**2.) for r_i in r], dtype=float)
        return hypgeom

    def alpha_radial(self, r, kappa_0, inverse_theta_c, slope = 8):
        """
        ...
        if inverse_theta_c == 0:
            return kappa_0 * r
        else:
            prefactor = 2./(2*slope -3) * kappa_0 / inverse_theta_c**2
            denominator_factor = (1 + r**2*inverse_theta_c**2)**(slope - 3./2)
            return prefactor/r * (1 - 1/denominator_factor)

    def derivatives(self, x, y, kappa_0, inverse_theta_c, slope=8, center_x=0, center_y=0):
        """
        ...
        f_x = self.alpha_radial(R, kappa_0, inverse_theta_c, slope) * x_ / R
        f_y = self.alpha_radial(R, kappa_0, inverse_theta_c, slope) * y_ / R
        return f_x, f_y

    def hessian(self, x, y, kappa_0, inverse_theta_c, slope=8, center_x=0, center_y=0):
        """
        :param x: angular position (normally in units of arc seconds)
        :param y: angular position (normally in units of arc seconds)
        :param kappa_0: central convergence of profile
        ...
        #  a_factor =  (0.5)**(-1./slope) -1
        if inverse_theta_c ==0:
            return kappa_0, kappa_0, 0
        else:
            prefactor = 2./(2*slope -3) * kappa_0 / inverse_theta_c**2
            # denominator factor
            denominator = 1 +  R**2*inverse_theta_c**2
            factor1 = (2*slope - 3) * denominator**(1./2 - slope) * inverse_theta_c**2 / R**2
    ...
    def density(self, R, kappa_0, inverse_theta_c, slope=8):
        """
        ...
        """
        rhotilde = self.rhotilde(kappa_0, inverse_theta_c, slope)
        #  a_factor =  (0.5)**(-1./slope) -1
        return rhotilde/(1 +  (R*inverse_theta_c)**2)**slope

    def density_lens(self, r, kappa_0, inverse_theta_c, slope=8):
        """
        ...
        """
        return self.density(r, kappa_0, inverse_theta_c, slope)

    def kappa_r(self, R, kappa_0, inverse_theta_c, slope=8):
        """
        ...
        :param inverse_theta_c: core radius
        :param slope: exponent entering the profile
        :return: convergence at r
        """
        #  a_factor =  (0.5)**(-1./slope) -1
        return kappa_0  * (1 +  (R*inverse_theta_c)**2)**(1./2 - slope)


    def density_2d(self, x, y, kappa_0, inverse_theta_c, slope=8, center_x=0, center_y=0):
        """
        ...
        y_ = y - center_y
        R = np.sqrt(x_**2 + y_**2)
        return self.kappa_r(R, kappa_0, inverse_theta_c, slope)
    ...
    def mass_3d(self, R, kappa_0, inverse_theta_c, slope=8):
        """
        ...
        rhotilde = self.rhotilde(kappa_0, inverse_theta_c, slope)
        #  a_factor =  (0.5)**(-1./slope) -1
        prefactor = 4. * np.pi * rhotilde / inverse_theta_c**3
        m_3d = prefactor * (self._mass_integral(R*inverse_theta_c, slope)
                - self._mass_integral(0, slope) )
        return m_3d

    def mass_3d_lens(self, r, kappa_0, inverse_theta_c, slope=8):
        """
        mass enclosed a 3d sphere or radius r
        ...
        """
        m_3d = self.mass_3d(r, kappa_0, inverse_theta_c, slope)
        return m_3d

    def mass_2d(self, R, kappa_0, inverse_theta_c, slope=8):
        """
        ...
        """
        m_2d = np.pi * R * self.alpha_radial(R, kappa_0, inverse_theta_c, slope)
        return m_2d
